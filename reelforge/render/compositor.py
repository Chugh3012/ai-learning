...
def _scene_clip(scene: Scene, style: Style, tts: TTS | None, tmp: Path, idx: int,
                opened: list) -> CompositeVideoClip:
    speech = None
...
def render(storyboard: Storyboard, out_path: str | Path, tts: TTS | None = None) -> Path:
    """Render a Storyboard to a vertical mp4. Pass a `tts` provider to add a synced voiceover
    (captions then follow the spoken word timings); omit it for evenly-timed silent captions."""
    ...
    video = None
    try:
        clips = [_scene_clip(s, style, tts, tmp, i, opened)
