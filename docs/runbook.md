...
# website (generated from topic packs: hub + one page per topic)
python web/build.py                                 # renders web/public/ from topics/*/pack.json
$tok = az staticwebapp secrets list -g rg-ai-scout -n swa-ai-scout --query properties.apiKey -o tsv
npx @azure/static-web-apps-cli deploy ./web/public --deployment-token $tok --env production
```
...
The site is generated by `web/build.py` from the topic packs: `/` is a hub listing every
publication and `/<topic>` is each pack's landing page (so `chughvibes.com/ai`,
`chughvibes.com/politics`). Adding a topic = add a pack, then rebuild — no new page code. The
API origin lives in `web/static/config.js` (`window.APP_CONFIG.apiBase`); per-topic copy lives in
each `topics/<id>/pack.json` under `settings.site`.

## Source quality dashboard

Every `prism.cli.sync` run refreshes `source-quality.md` — uploaded to the KB Blob container
...
a topic's `topics/<id>/sources.opml`.
...
up). Restore with the `prism.cli.restore` command. Set the account first:

```pwsh
$env:STORAGE_ACCOUNT = "staiscoutv9uothke"; $env:BLOB_CONTAINER = "knowledge"

python -m prism.cli.restore --list              # list available snapshots
python -m prism.cli.restore                      # restore the latest snapshot
python -m prism.cli.restore 2026-06-17T23:19:35.4Z   # restore a specific snapshot
```

Validate the restored KB **before** trusting it:

```pwsh
python -c "from prism.repositories.knowledge import KnowledgeBase; kb=KnowledgeBase.open('data/kb/kb.sqlite'); print(kb.metrics_snapshot()); kb.close()"
```
...
```pwsh
python -m prism.cli.sync --no-upload
```
