...
from prism.repositories import models
...
class KnowledgeBase:
    ...
    def open(cls, path=None) -> "KnowledgeBase":
        ...
        cls._backfill_topic(con)
        return cls(engine, con)

    @staticmethod
    def _backfill_topic(con: sqlite3.Connection) -> None:
        # Existing rows predate topic_id; treat them as the original AI topic.
        for table in ("source", "item"):
            try:
                con.execute(f"UPDATE {table} SET topic_id='ai' WHERE topic_id IS NULL")
    ...
    def upsert_source(self, s: dict) -> int:
        self.con.execute(
            "INSERT INTO source(title,url,kind,category,topic_id) VALUES(?,?,?,?,?) "
            "ON CONFLICT(url) DO UPDATE SET title=excluded.title, category=excluded.category, "
            "topic_id=excluded.topic_id",
            (s["title"], s["url"], "feed", s["category"], s.get("topic_id", "ai")),
        )
    ...
    def insert_item(self, source_id: int, title: str, url: str, summary: str,
                    published: int, now: int, h: str, topic_id: str = "ai") -> int | None:
        cur = self.con.execute(
            "INSERT OR IGNORE INTO item(source_id,title,url,summary,published,fetched_at,hash,topic_id) "
            "VALUES(?,?,?,?,?,?,?,?)",
            (source_id, title, url, summary, published, now, h, topic_id),
        )
        if not cur.rowcount:
            return None
    ...
    def unscored_recent(self, days: int, max_items: int, topic_id: str | None = None) -> list[tuple]:
        cutoff = int(time.time()) - days * 86400
        where = ("i.published >= ? AND NOT EXISTS "
                 "(SELECT 1 FROM signal s WHERE s.item_id=i.id AND s.kind='relevance')")
        params: list = [cutoff]
        if topic_id:
            where += " AND i.topic_id=?"
            params.append(topic_id)
        params.append(max_items)
        return self.con.execute(
            f"SELECT i.id, i.title, i.summary FROM item i WHERE {where} "
    ...
    def candidates(self, lens: str, limit: int, topic_id: str | None = None) -> list[tuple]:
